...
Tôi đã viết các kỹ thuật trong chương này để thu hẹp khoảng cách. Thường thì chúng ta phải suy luận về các tác động theo những cách khác thường để tìm ra nơi tốt nhất để kiểm thử.
...
> IDE hỗ trợ việc phân tích tác động
> Đôi khi tôi ước mình có một IDE hỗ trợ tôi biết được các tác động trong code kế thừa. Tôi có thể đánh dấu một đoạn code và nhấn một phím nóng. Sau đó, IDE sẽ cung cấp cho tôi danh sách tất cả các biến và phương thức có thể bị ảnh hưởng khi tôi thay đổi đoạn code đã chọn.
> Có lẽ một ngày nào đó sẽ có người phát triển một công cụ như vậy. Trong khi chờ đợi, chúng ta phải suy luận các tác động mà không có công cụ. Đó là một kỹ năng rất dễ học, nhưng thật khó để biết khi nào chúng ta đã hiểu đúng.

Cách tốt nhất để hiểu được lý luận về tác động có tác dụng như thế nào là xem xét một ví dụ. Xem xét một lớp Java là một phần của ứng dụng thao tác ngôn ngữ C++. Nghe có vẻ khá chuyên sâu về miền phải không? Nhưng kiến thức miền không quan trọng khi chúng ta suy luận các tác động.
...
> Nếu code của bạn được cấu trúc tốt, hầu hết các phương thức trong ứng dụng của bạn đều có cấu trúc tác động đơn giản. Trên thực tế, một thước đo mức độ tốt của ứng dụng là các tác động khá phức tạp đối với thế giới bên ngoài là tổng của một tập hợp các tác động đơn giản hơn nhiều bên trong code. Hầu như bất cứ điều gì bạn có thể làm khiến cho bản phác thảo tác động đơn giản hơn cho một đoạn code đều khiến nó trở nên dễ hiểu và dễ bảo trì hơn.

Hãy mở rộng bức tranh của chúng ta về hệ thống mà lớp trước hình thành và nhìn vào một bức tranh tác động lớn hơn. Các đối tượng `CppClass` được tạo trong một lớp có tên là `ClassReader`. Trên thực tế, chúng tôi có thể xác định rằng chúng chỉ được tạo trong `ClassReader`.
...
Trong ví dụ này, điều đầu tiên chúng ta cần làm là tìm ra nơi chúng ta sẽ thực hiện các thay đổi của mình. Chúng ta cần xóa chức năng khỏi `generateIndex` và thêm chức năng vào `addElement`. Khi chúng ta đã xác định đó là những điểm thay đổi, chúng ta có thể bắt đầu phác thảo các tác động.
...
> Khi bạn đang phác thảo các tác động, hãy đảm bảo rằng bạn đã tìm thấy tất cả vị trí sử dụng của lớp bạn đang kiểm tra. Nếu lớp của bạn có một lớp cha hoặc các lớp con, thì có thể có vị trí khác mà bạn chưa xem xét.

Bây giờ thì sao? Chà, có một điều mà chúng tôi đã che đậy hoàn toàn. Chúng tôi đang sử dụng lớp `Element` trong thư mục, nhưng nó không nằm trong bản phác thảo tác động của chúng tôi. Hãy xem xét nó kỹ hơn.
...
Có một số cách lan truyền tác động dễ nhận thấy hơn những cách khác. Trong ví dụ về `InMemoryDirectory` ở phần trước, cuối cùng chúng ta cùng tìm thấy các phương thức trả về giá trị cho trình gọi. Mặc dù bắt đầu bằng cách theo dõi các tác động từ các điểm thay đổi, những nơi đang thực hiện thay đổi, nhưng tôi cũng thường chú ý đến các phương thức có giá trị trả về trước tiên. Trừ khi các giá trị trả về của chúng không được sử dụng, chúng sẽ truyền các tác động tới code gọi chúng.

Các tác động cũng có thể lan truyền theo những cách thầm lặng, lén lút. Nếu chúng ta có một đối tượng nhận một số đối tượng khác làm tham số, nó có thể sửa đổi trạng thái của nó và thay đổi sẽ phản ánh trở lại phần còn lại của ứng dụng.
...
> Các tác động lan truyền trong code theo ba cách cơ bản:
...
Đây là kinh nghiệm tôi sử dụng khi tìm kiếm các tác động:
...
Khi chúng ta loại bỏ các phần trùng lặp nhỏ, chúng ta thường thu được bản phác thảo tác động với tập hợp các điểm cuối nhỏ hơn. Điều này thường giúp quyết định kiểm thử dễ dàng hơn.
